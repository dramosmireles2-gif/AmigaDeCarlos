...
export default function NuevoProducto() {
  const [variants, setVariants] = useState([{ color: '', colorHex: '#000000', stock: 0 }])

  function addVariant() {
    setVariants(v => [...v, { color: '', colorHex: '#000000', stock: 0 }])
  }
      ...
      <form className="space-y-8">
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-6 space-y-4">
          ...
          <div>
            ...
            <input required placeholder="Ej: Bolsa Mini Noir" className="w-full border border-stone-200 px-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-stone-400" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-stone-500 uppercase tracking-wider block mb-1.5">Categoría</label>
              <select required className="w-full border border-stone-200 px-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-stone-400 bg-white">
            ...
            <div>
              ...
              <input placeholder="bolsa-mini-noir" className="w-full border border-stone-200 px-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-stone-400" />
            </div>
          </div>
          <div>
            <label className="text-xs text-stone-500 uppercase tracking-wider block mb-1.5">Descripción</label>
            <textarea rows={3} placeholder="Describe el producto..." className="w-full border border-stone-200 px-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-stone-400 resize-none" />
          </div>
          <div className="flex items-center gap-2">
            <input type="checkbox" id="isNew" className="accent-stone-900 w-4 h-4" />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" required min="0" placeholder="0.00" className="w-full border border-stone-200 pl-8 pr-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-stone-400" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-stone-400 text-sm">$</span>
                <input type="number" min="0" placeholder="0.00" className="w-full border border-stone-200 pl-8 pr-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-stone-400" />
              </div>
            </div>
          </div>
        </div>
              ...
              <div key={i} className="grid grid-cols-3 gap-3 items-end">
                <div>
                  ...
                  <input
                    placeholder="Negro"
                    value={v.color}
                    onChange={e => setVariants(prev => prev.map((pv, pi) => pi === i ? { ...pv, color: e.target.value } : pv))}
                ...
                <div>
                  ...
                  <input
                    type="color"
                    value={v.colorHex}
                    onChange={e => setVariants(prev => prev.map((pv, pi) => pi === i ? { ...pv, colorHex: e.target.value } : pv))}
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      min="0"
                      value={v.stock}
                      onChange={e => setVariants(prev => prev.map((pv, pi) => pi === i ? { ...pv, stock: parseInt(e.target.value) || 0 } : pv))}
                      className="w-full border border-stone-200 px-3 py-2 text-sm rounded-lg focus:outline-none focus:border-stone-400"
                    />
                  </div>
                  {variants.length > 1 && (
                    <button type="button" onClick={() => removeVariant(i)} className="mb-0.5 text-stone-300 hover:text-red-500 transition-colors self-end pb-2">
